...
function HowItWorksIcon({ type }: { type: "find" | "join" | "chat" | "feedback" | "grow" }) {
  if (type === "find") return <svg viewBox="0 0 48 48" aria-hidden="true"><path d="M24 43s13-11.2 13-25a13 13 0 1 0-26 0c0 13.8 13 25 13 25Z" /><circle cx="24" cy="18" r="5" /><path d="M34 35h9M39 30v10" /></svg>;
  if (type === "join") return <svg viewBox="0 0 48 48" aria-hidden="true"><circle cx="18" cy="16" r="6" /><circle cx="34" cy="19" r="5" /><path d="M6 38c1-8 5-12 12-12s11 4 12 12M29 29c7-1 11 2 13 9" /><path d="m31 9 3 3 6-7" /></svg>;
  if (type === "chat") return <svg viewBox="0 0 48 48" aria-hidden="true"><path d="M7 8h34v24H21l-10 8v-8H7V8Z" /><path d="M14 17h20M14 24h13" /><circle cx="37" cy="36" r="5" /></svg>;
  if (type === "feedback") return <svg viewBox="0 0 48 48" aria-hidden="true"><path d="m24 6 5.2 10.5 11.6 1.7-8.4 8.2 2 11.6L24 32.5 13.6 38l2-11.6-8.4-8.2 11.6-1.7L24 6Z" /><path d="m17 25 4 4 10-11" /></svg>;
  return <svg viewBox="0 0 48 48" aria-hidden="true"><path d="M7 40V28M17 40V21M27 40V14" /><path d="m8 20 9-8 8 5L40 5" /><path d="M32 5h8v8" /><circle cx="37" cy="35" r="7" /></svg>;
}
  ...
  return (
    <main className={`shell ${settingsOpen || notificationsOpen || howItWorksOpen || calendarOpen || connectionsOpen || cmrDetailsOpen ? "utility-page-open" : ""} ${workspaceGroup || viewedGroup ? "detail-page-open" : ""} ${rankingGame ? "ranking-page-open" : ""}`}>
      ...
      {activeTab === "games" && <section className="page-view games-page" onClickCapture={(event) => {
        ...
        {(gamesViewTab === "upcoming" || gamesViewTab === "pending") && <div className="games-list-search-row"><label className="games-list-search"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="11" cy="11" r="6.5" /><path d="m16 16 4 4" /></svg><span className="sr-only">Search {gamesViewTab} games</span><input type="search" value={gamesViewTab === "upcoming" ? upcomingSearch : pendingSearch} onChange={(event) => gamesViewTab === "upcoming" ? setUpcomingSearch(event.target.value) : setPendingSearch(event.target.value)} placeholder={`Search ${gamesViewTab} games`} autoComplete="off" />{(gamesViewTab === "upcoming" ? upcomingSearch : pendingSearch) && <button type="button" onClick={() => gamesViewTab === "upcoming" ? setUpcomingSearch("") : setPendingSearch("")} aria-label={`Clear ${gamesViewTab} search`}>Clear</button>}</label><small>{gamesViewTab === "upcoming" ? `${filteredUpcomingGames.length} of ${upcomingGames.length}` : `${filteredRequestedGames.length} of ${requestedGames.length}`} games</small></div>}
      ...
      {howItWorksOpen && user && <section className="utility-page how-it-works-page" aria-labelledby="how-it-works-title">
        ...
        <section className="how-it-works-lead"><div className="how-it-works-lead-art" aria-hidden="true"><span className="guide-orbit orbit-one" /><span className="guide-orbit orbit-two" /><span className="guide-main-icon"><HowItWorksIcon type="find" /></span><i>CMR</i><b>7 PM</b></div><div><span className="how-step-label">01 · FIND OR CREATE</span><h2>Choose your next game</h2><p>Tell Ask what you need, or browse the map.</p><div className="how-it-works-signals"><b>SPORT</b><b>AREA</b><b>TIME</b><b>CMR</b></div></div></section>
        <div className="how-it-works-grid" aria-label="CourtMate game journey">
          <section><div className="how-card-top"><span>02</span><span className="how-card-icon"><HowItWorksIcon type="join" /></span></div><h2>Join the lineup</h2><p>Request a spot. The organizer confirms.</p></section>
          <section><div className="how-card-top"><span>03</span><span className="how-card-icon"><HowItWorksIcon type="chat" /></span></div><h2>Chat, then play</h2><p>Coordinate together in Group Space.</p></section>
          <section className="how-it-works-feedback"><div className="how-card-top"><span>04</span><span className="how-card-icon"><HowItWorksIcon type="feedback" /></span></div><h2>Complete and rate</h2><p>Mark it complete. Rate your players.</p></section>
          <section className="how-it-works-cmr"><div className="how-card-top"><span>05</span><span className="how-card-icon"><HowItWorksIcon type="grow" /></span></div><h2>Watch your CMR grow</h2><p>Every sport keeps its own progress.</p></section>
        </div>
        <div className="how-it-works-result"><span className="how-result-score"><strong>8.6</strong><small>BADMINTON CMR</small></span><div><strong>Your player story builds here</strong><span>Games · CMR · streak · leaderboard</span></div></div>
