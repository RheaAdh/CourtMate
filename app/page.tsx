...
import { createStreakShareCard, shareImageFile, SocialFeed } from "./social-feed";
...
function isPerformanceQuery(query: string): boolean {
  ...
  return /\b(cmr|rating|ratings|feedback|review|stats|statistics|calories|steps|heart rate|distance|wearable|progress|trend|fitness|form)\b/.test(normalized)
  ...
  return (
    <main className={`shell ${settingsOpen || notificationsOpen || calendarOpen || connectionsOpen || cmrDetailsOpen ? "utility-page-open" : ""} ${workspaceGroup || viewedGroup ? "detail-page-open" : ""} ${rankingGame ? "ranking-page-open" : ""}`}>
      ...
      <section className="guest-home" aria-label="CourtMate introduction">
        ...
        <section className="guest-cmr-section" aria-labelledby="guest-cmr-title"><div className="guest-cmr-heading"><span className="eyebrow">A RATING THAT TRAVELS WITH YOU</span><h2 id="guest-cmr-title">Meet your CMR.</h2><p>CMR means CourtMate Rating: one evolving skill signal for each racket sport. Everyone starts at 1.0; only confirmed competitive results move it.</p></div><div className="guest-cmr-steps"><article><strong>01</strong><h3>Start simple</h3><p>Your first rating is a baseline, not a label you need to prove before playing.</p></article><article><strong>02</strong><h3>Build real evidence</h3><p>Confirmed results consider both sides, the outcome, and a bounded score margin. Casual games still build history and streaks.</p></article><article><strong>03</strong><h3>Find better fits</h3><p>Carry your CMR across groups so organizers and players can match the pace of the game.</p></article></div></section>
