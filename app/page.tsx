...
import { BENGALURU_AREA_GROUPS, BENGALURU_AREA_NAMES, CommunityHub, type NearbyGame } from "./community-hub";
...
const PENDING_SHARED_GAME_KEY = "courtmate:pending-shared-game";

const compareSessionDateDesc = (
  left: { session_date: string; start_time: string },
  right: { session_date: string; start_time: string },
) => `${right.session_date} ${right.start_time}`.localeCompare(`${left.session_date} ${left.start_time}`);

const compareUpcomingGames = (
  left: { created_at?: string | null; session_date: string; start_time: string },
  right: { created_at?: string | null; session_date: string; start_time: string },
) => {
  ...
  return compareSessionDateDesc(left, right);
};
...
function sharedGameIdFromBrowser() {
  const currentUrl = new URL(window.location.href);
  const linkedGameId = currentUrl.searchParams.get("rally-circle");
  if (linkedGameId) {
    try { window.sessionStorage.setItem(PENDING_SHARED_GAME_KEY, linkedGameId); } catch { /* Storage is optional. */ }
    return linkedGameId;
  }
  try {
    const pendingGameId = window.sessionStorage.getItem(PENDING_SHARED_GAME_KEY);
    if (pendingGameId) {
      currentUrl.searchParams.set("rally-circle", pendingGameId);
...
type CircleLeaderboardPeriod = "week" | "30_days" | "3_months" | "year";
type CircleLeaderboardMetric = "sessions" | "cmr";
...
function isCreationConfirmation(query: string): boolean {
  const normalized = query.toLowerCase()
    .replace(/[.!?]+$/g, "")
    .replace(/\bplease\b/g, "")
    .replace(/\s+/g, " ")
    .trim();
  return /^(?:create(?: (?:this |the )?game| it)?(?: now)?|yes(?:,? create (?:it|the game))?|looks good|go ahead|use that|confirm|all set|that's good)$/.test(normalized);
}
...
export default function Home() {
  ...
  const [profileDraft, setProfileDraft] = useState<ProfileDraft>({ bio: "", is_profile_private: false, default_session_visibility: "public", area: "Whitefield", age: "", gender: "", preferred_age_range: "any", preferred_genders: [], travel_radius_km: "10", availability: [] });
  ...
  const [exploreAreaFilter, setExploreAreaFilter] = useState("all");
  ...
  const [exploreAreaApplied, setExploreAreaApplied] = useState("all");
  ...
  const [circleLeaderboardMetric, setCircleLeaderboardMetric] = useState<CircleLeaderboardMetric>("sessions");
  const [circleLeaderboardGamesSport, setCircleLeaderboardGamesSport] = useState<Sport | "all">("all");
  const [circleLeaderboardSport, setCircleLeaderboardSport] = useState<Sport>("pickleball");
  const [circleLeaderboardPeriod, setCircleLeaderboardPeriod] = useState<CircleLeaderboardPeriod>("30_days");
  ...
  useEffect(() => {
    if (!settingsOpen && !notificationsOpen && !connectionsOpen && !howItWorksOpen) return;
    const closeOnEscape = (event: KeyboardEvent) => {
      if (event.key === "Escape") closeUtilityPage();
    };
    window.addEventListener("keydown", closeOnEscape);
    return () => window.removeEventListener("keydown", closeOnEscape);
  }, [settingsOpen, notificationsOpen, connectionsOpen, howItWorksOpen]);
  ...
  useEffect(() => {
    const syncUtilityPage = () => {
      ...
      setHowItWorksOpen(hash === "how-it-works");
  ...
  useEffect(() => {
    ...
    return onAuthStateChanged(auth, (nextUser) => {
      ...
      if (nextUser) {
        ...
        const hasSharedRallyCircle = Boolean(sharedGameIdFromBrowser());
  ...
  useEffect(() => {
    ...
    const sharedGameId = sharedGameIdFromBrowser();
    ...
    setActiveTab("games");
    void openSharedGame(sharedGameId, user).finally(() => {
      try { window.sessionStorage.removeItem(PENDING_SHARED_GAME_KEY); } catch { /* Storage is optional. */ }
    });
  }, [authReady, user]);
  ...
  useEffect(() => {
    ...
    void loadCircleLeaderboard(circleLeaderboardScope, circleLeaderboardMetric, circleLeaderboardGamesSport, circleLeaderboardSport, circleLeaderboardPeriod, user, true);
    const refresh = () => {
      if (document.visibilityState === "visible") void loadCircleLeaderboard(circleLeaderboardScope, circleLeaderboardMetric, circleLeaderboardGamesSport, circleLeaderboardSport, circleLeaderboardPeriod, user, false);
    };
    const interval = window.setInterval(refresh, 5000);
    return () => window.clearInterval(interval);
  }, [activeTab, circleLeaderboardScope, circleLeaderboardMetric, circleLeaderboardGamesSport, circleLeaderboardSport, circleLeaderboardPeriod, user]);
  ...
  async function loadCircleLeaderboard(scope: CircleLeaderboardScope, metric: CircleLeaderboardMetric, gamesSport: Sport | "all", sport: Sport, period: CircleLeaderboardPeriod, authUser: User = user as User, showLoader = true) {
    if (!authUser) return;
    ...
    try {
      ...
      const gamesSportQuery = metric === "sessions" && gamesSport !== "all" ? `&session_sport=${gamesSport}` : "";
      const response = await authorizedFetch(`${apiUrl}/v1/me/circle-leaderboard?scope=${scope}&metric=${metric}&sport=${sport}&period=${period}${gamesSportQuery}`, {}, authUser);
  ...
  async function toggleFollowProfile() {
    ...
    try {
      ...
      setToast(wasFollowing ? "Player unfollowed" : nextProfile.follow_request_pending ? "Follow request sent" : "Now following");
      window.setTimeout(() => setToast(""), 2600);
    } catch {
  ...
  async function toggleConnection(connection: PublicPlayerProfile) {
    ...
    try {
      ...
      if (wasFollowing !== updated.is_following) {
        setSocialProfile((current) => current ? { ...current, following_count: Math.max(0, current.following_count + (updated.is_following ? 1 : -1)) } : current);
      }
      void refreshLiveStateFor(["connections", "social-profile", "feed", "recommendations"]);
      setToast(wasFollowing ? "Player unfollowed" : updated.follow_request_pending ? "Follow request sent" : "Now following");
      window.setTimeout(() => setToast(""), 2600);
    } catch {
  ...
  async function clearReadNotifications() {
    if (!user) return;
    try {
      const response = await authorizedFetch(`${apiUrl}/v1/me/notifications/read`, { method: "DELETE" });
      if (!response.ok) throw new Error("Notification cleanup failed");
      const payload = await response.json() as { notifications: AppNotification[] };
      setNotifications(payload.notifications);
      setToast("Read notifications cleared");
      window.setTimeout(() => setToast(""), 2600);
    } catch {
      setToast("Could not clear read notifications");
      window.setTimeout(() => setToast(""), 2600);
    }
  }
  ...
  async function signOutUser() {
    ...
    setProfile(null);
    setProfileDraft({ bio: "", is_profile_private: false, default_session_visibility: "public", area: "Whitefield", age: "", gender: "", preferred_age_range: "any", preferred_genders: [], travel_radius_km: "10", availability: [] });
  ...
  async function saveProfile(event?: FormEvent) {
    ...
    const body: { bio: string; is_profile_private: boolean; default_session_visibility: SessionVisibility; area: string; availability: string[]; travel_radius_km: number; preferred_age_range: AgeRange; preferred_genders: Gender[]; age?: number; gender?: Gender; latitude?: number; longitude?: number } = {
    ...
    try {
      ...
      setProfileDraft({ bio: updatedProfile.bio ?? "", is_profile_private: updatedProfile.is_profile_private ?? false, default_session_visibility: updatedProfile.default_session_visibility ?? "public", area: updatedProfile.area, age: updatedProfile.age?.toString() ?? "", gender: updatedProfile.gender ?? "", preferred_age_range: updatedProfile.preferred_age_range ?? "any", preferred_genders: updatedProfile.preferred_genders ?? [], latitude: updatedProfile.latitude, longitude: updatedProfile.longitude, travel_radius_km: updatedProfile.travel_radius_km?.toString() ?? "10", availability: updatedProfile.availability ?? [] });
  ...
  function parseCreationReply(reply: string) {
    ...
    const cmrRange = lowered.match(/\b(?:cmr\s*)?(10|[1-9](?:\.\d+)?)\s*(?:-|to|–)\s*(10|[1-9](?:\.\d+)?)\b/);
    if (cmrRange) {
      const minimum = clampCmr(Number(cmrRange[1]));
      const maximum = clampCmr(Number(cmrRange[2]));
      if (minimum <= maximum) {
        nextDraft.skill_min = minimum.toString();
        nextDraft.skill_max = maximum.toString();
        changed = true;
      }
    } else if (creationStep === "skill" && /\b(?:my (?:cmr|level)|use my (?:cmr|level)|similar to me)\b/.test(lowered)) {
      const playerCmr = cmrForSport(nextDraft.sport);
      nextDraft.skill_min = clampCmr(playerCmr - 1.8).toString();
      nextDraft.skill_max = clampCmr(playerCmr + 1.8).toString();
      changed = true;
    } else if (creationStep === "skill" && /\b(?:any|all) (?:cmr|level|skill)\b/.test(lowered)) {
      nextDraft.skill_min = "1";
      nextDraft.skill_max = "10";
      changed = true;
    } else if (creationStep === "skill") {
      const singleCmr = lowered.match(/^\s*(?:cmr\s*)?(10|[1-9](?:\.\d+)?)\s*$/);
      if (singleCmr) {
        const center = clampCmr(Number(singleCmr[1]));
        nextDraft.skill_min = clampCmr(center - 1).toString();
        nextDraft.skill_max = clampCmr(center + 1).toString();
        changed = true;
      }
    }
    ...
    const useSuggestedName = creationStep === "name" && /^(?:(?:use|keep) (?:the )?(?:suggested|default)(?: name)?|suggested|default|no|no thanks|skip|that's fine)$/i.test(normalizedReply);
  ...
  async function handleCreationReply(reply: string) {
    ...
    if (creationStep === "confirm" && isCreationConfirmation(cleanReply)) {
      await createGroup();
      return;
    }
  ...
  function routeAskMessage(message: string) {
    ...
    if (groupProposal && creationStep === "confirm" && isCreationConfirmation(cleanMessage)) {
  ...
  function suggestedPrompts() {
    const initialPrompts = ["Find games around me", "Create a game", "How is my CMR changing?"];
    const hasActiveConversation = chatMessages.length > 0 || sessions.length > 0 || Boolean(groupProposal || discoveryStep || feedbackSessionId || feedbackPickerOpen || scoreSessionId || scorePickerOpen);
    if (!hasActiveConversation) return initialPrompts;

    const lastUserMessage = [...chatMessages].reverse().find((message) => message.role === "user")?.text.trim().toLowerCase();
    return [...new Set(quickPrompts())]
      .filter((prompt) => prompt.trim().toLowerCase() !== lastUserMessage)
      .slice(0, 3);
  }
  ...
  async function createGroup() {
    ...
    const fail = (message: string, correctionStep?: CreationStep) => {
      setCreateGroupError(message);
      setToast(message);
      if (groupProposal) {
        if (correctionStep) {
          setCreationStep(correctionStep);
          setShowCraftedGame(false);
        } else {
          setCreationStep("confirm");
          setShowCraftedGame(true);
        }
        const followUp = correctionStep
          ? `${message} ${creationQuestion(correctionStep)}`
          : `I couldn't create this game yet: ${message} Your plan is still here, so you can try again.`;
        setChatMessages((messages) => [...messages.slice(-8), { id: `${Date.now()}-creation-error`, role: "assistant", text: followUp }]);
      }
    };
    if (!user) {
    ...
    if (!createGroupDraft.session_date || createGroupDraft.session_date < today) {
      fail("Choose today or a future game date.", "date");
      return;
    }
    if (!createGroupDraft.start_time || !createGroupDraft.end_time) {
      fail("Choose the start and end time for the game.", "time");
      return;
    }
    if (createGroupDraft.end_time <= createGroupDraft.start_time) {
      fail("End time must be after start time.", "time");
      return;
    }
    ...
    if (windowMinutes < 60) {
      fail("Choose a game window of at least one hour.", "time");
      return;
    }
    if (createGroupDraft.session_date === today && createGroupDraft.start_time <= localTimeInput()) {
      fail("Choose a future start time, or select tomorrow.", "time");
      return;
    }
  ...
  async function deleteCreatedGame(game: Pick<Session, "id" | "group_name">) {
    if (deletingGameId || !window.confirm(`Delete “${game.group_name}”? This cannot be undone.`)) return;
    const previousGroups = myGroups;
    const previousApproved = approvedGames;
    const previousIncoming = incomingRequests;
    setDeletingGameId(game.id);
    setMyGroups((groups) => groups.filter((item) => item.id !== game.id));
    setApprovedGames((games) => games.filter((item) => item.id !== game.id));
    setIncomingRequests((requests) => requests.filter((item) => item.session.id !== game.id));
    if (workspaceGroup?.id === game.id) setWorkspaceGroup(null);
    try {
      const response = await authorizedFetch(`${apiUrl}/v1/sessions/${encodeURIComponent(game.id)}`, { method: "DELETE" });
      const payload = await response.json().catch(() => ({})) as { detail?: string };
      if (!response.ok) throw new Error(payload.detail ?? "Unable to delete game");
      setToast("Game deleted");
      void loadGamesActivity(user, false, false, true);
    } catch (error) {
      setMyGroups(previousGroups);
      setApprovedGames(previousApproved);
      setIncomingRequests(previousIncoming);
      setToast(error instanceof Error ? error.message : "Unable to delete game");
    } finally {
  ...
  function gameInviteMessage(game: ShareableGame) {
    const gameDate = new Date(`${game.session_date}T12:00:00`);
    const weekday = gameDate.toLocaleDateString("en-IN", { weekday: "long" });
    const month = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"][gameDate.getMonth()];
    const dateLabel = `${weekday}, ${gameDate.getDate()} ${month}`;
    ...
    return `${sportEmoji[game.sport]} ${game.group_name}\n📅 ${dateLabel}\n⏰ ${game.start_time.slice(0, 5)} - ${game.end_time.slice(0, 5)}\n📍 ${location}\n🔗 Join Game: ${groupSpaceUrl}`;
  }

  async function copyGroupSpaceLink(game: ShareableGame) {
    try {
      await copyText(getGroupSpaceUrl(game));
      setToast("Copied link to clipboard");
    } catch {
      setToast("Could not copy the game link");
    }
    window.setTimeout(() => setToast(""), 2600);
  }

  async function shareGame(game: ShareableGame) {
    const title = `Join ${game.group_name} on CourtMate`;
    const message = gameInviteMessage(game);
    const groupSpaceUrl = getGroupSpaceUrl(game);
    try {
      if (isDesktopShareView()) {
      ...
      try {
        ...
        showToast("Could not copy the game link");
      }
    }
  }
  ...
  function selectTab(tab: AppTab) {
    ...
    setRankingGame(null);
    setViewedGroup(null);
    if (["#settings", "#notifications", "#profile-calendar", "#connections", "#how-it-works"].includes(window.location.hash)
  ...
  function openHowItWorks() {
    if (!user) {
      document.querySelector(".guest-story")?.scrollIntoView({ behavior: "smooth" });
      return;
    }
    window.history.pushState({ courtMatePage: "how-it-works" }, "", "#how-it-works");
  ...
  const requestedGames = myRequests
    .filter(({ request }) => request.status === "pending" || request.status === "waitlisted")
    .sort((left, right) => compareSessionDateDesc(left.session, right.session));
  ...
  const upcomingGames = Array.from(new Map([...approvedGames, ...myGroups.filter((group) => group.session_date >= today && group.status !== "awaiting_feedback" && group.status !== "completed" && group.status !== "cancelled")].map((game) => [game.id, game])).values()).sort(compareUpcomingGames);
  const upcomingSearchTerms = upcomingSearch.trim().toLowerCase().split(/\s+/).filter(Boolean);
  const pendingSearchTerms = pendingSearch.trim().toLowerCase().split(/\s+/).filter(Boolean);
  const filteredUpcomingGames = upcomingGames.filter((game) => {
    const searchableGame = `${game.group_name} ${sportLabel(game.sport)} ${game.area} ${game.session_date} ${game.start_time} ${game.end_time} ${game.venue_name ?? ""}`.toLowerCase();
    return upcomingSearchTerms.every((term) => searchableGame.includes(term));
  });
  const filteredRequestedGames = requestedGames.filter(({ request, session }) => {
    const searchableGame = `${session.group_name} ${sportLabel(session.sport)} ${session.area} ${session.session_date} ${session.start_time} ${session.end_time} ${session.venue_name ?? ""} ${request.status}`.toLowerCase();
    return pendingSearchTerms.every((term) => searchableGame.includes(term));
  });
  const exploreBrowseGames = Array.from(new Map([
    ...exploreGames,
    ...myGroups
      .filter((game) => game.session_date >= today && ["open", "full", "in_progress"].includes(game.status) && game.visibility !== "private")
      .map((game): Session => ({
        ...game,
        game_format: game.game_format ?? "doubles",
        open_slots: Math.max(game.capacity - game.confirmed_player_ids.length, 0),
        score: 1,
        explanation: "You organize this game.",
      })),
  ].map((game) => [game.id, game])).values());
  const filteredExploreGames = exploreBrowseGames.filter((game) => {
    ...
    if (exploreAreaApplied !== "all" && game.area.trim().toLowerCase() !== exploreAreaApplied.toLowerCase()) return false;
  ...
  const sortedAwaitingFeedbackGames = [...awaitingFeedbackGames].sort(compareSessionDateDesc);
  const sortedPastGames = [...pastGames].sort((left, right) => compareSessionDateDesc(left.session, right.session));
  const sortedApprovedGames = [...approvedGames].sort(compareSessionDateDesc);
  const sortedMyGroups = [...myGroups].sort(compareSessionDateDesc);
  const applyExploreFilters = () => {
    setExploreSearch(exploreSearchDraft.trim());
    setExploreSportApplied(exploreSportFilter);
    setExploreAreaApplied(exploreAreaFilter);
  ...
  return (
    <main className={`shell ${settingsOpen || notificationsOpen || howItWorksOpen || calendarOpen || connectionsOpen || cmrDetailsOpen ? "utility-page-open" : ""} ${workspaceGroup || viewedGroup ? "detail-page-open" : ""} ${rankingGame ? "ranking-page-open" : ""}`}>
      <nav className="nav">
      ...
      {activeTab === "games" && <section className="page-view games-page" onClickCapture={(event) => {
          ...
          <div className="game-create-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) closeCreateGameForm(); }}>
            <form className="game-create-form game-create-quick-form" onSubmit={(event) => { event.preventDefault(); void createGroup(); }}>
              ...
              <div className="game-create-essentials">
                ...
                <label><span>Area</span><select value={createGroupDraft.area} onChange={(event) => setCreateGroupDraft({ ...createGroupDraft, area: event.target.value })} required><option value="" disabled>Choose area</option>{createGroupDraft.area && !BENGALURU_AREA_NAMES.includes(createGroupDraft.area) && <option value={createGroupDraft.area}>{createGroupDraft.area}</option>}{BENGALURU_AREA_GROUPS.map((group) => <optgroup label={group.label} key={group.label}>{group.areas.map((area) => <option value={area} key={area}>{area}</option>)}</optgroup>)}</select></label>
        ...
        {(gamesViewTab === "upcoming" || gamesViewTab === "pending") && <div className="games-list-search-row"><label className="games-list-search"><span className="games-search-icon" aria-hidden="true"><svg viewBox="0 0 24 24"><circle cx="11" cy="11" r="6.5" /><path d="m16 16 4 4" /></svg></span><input type="search" value={gamesViewTab === "upcoming" ? upcomingSearch : pendingSearch} onChange={(event) => gamesViewTab === "upcoming" ? setUpcomingSearch(event.target.value) : setPendingSearch(event.target.value)} placeholder={`Search ${gamesViewTab} games`} aria-label={`Search ${gamesViewTab} games`} autoComplete="off" /><span className="games-search-count" aria-live="polite">{gamesViewTab === "upcoming" ? `${filteredUpcomingGames.length}/${upcomingGames.length}` : `${filteredRequestedGames.length}/${requestedGames.length}`}<small>games</small></span>{(gamesViewTab === "upcoming" ? upcomingSearch : pendingSearch) && <button type="button" onClick={() => gamesViewTab === "upcoming" ? setUpcomingSearch("") : setPendingSearch("")} aria-label={`Clear ${gamesViewTab} search`}>×</button>}</label></div>}
        {gamesViewTab === "explore" && <div className="explore-filters" aria-label="Filter games"><label className="explore-filter-search"><span>Search games</span><input value={exploreSearchDraft} onChange={(event) => setExploreSearchDraft(event.target.value)} placeholder="Name, sport, or area" onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); applyExploreFilters(); } }} /></label><label><span>Sport</span><select value={exploreSportFilter} onChange={(event) => setExploreSportFilter(event.target.value as Sport | "all")}><option value="all">All sports</option>{sportOptions.map((sport) => <option value={sport.value} key={sport.value}>{sport.label}</option>)}</select></label><label><span>Area</span><select value={exploreAreaFilter} onChange={(event) => setExploreAreaFilter(event.target.value)}><option value="all">All areas</option>{BENGALURU_AREA_GROUPS.map((group) => <optgroup label={group.label} key={group.label}>{group.areas.map((area) => <option value={area} key={area}>{area}</option>)}</optgroup>)}</select></label><label><span>Skill level (CMR)</span><select value={exploreCmrFilter} onChange={(event) => setExploreCmrFilter(event.target.value as ExploreCmrFilter)}><option value="all">Any CMR</option><option value="beginner">Beginner · 1.0–2.9</option><option value="intermediate">Intermediate · 3.0–5.9</option><option value="advanced">Advanced · 6.0–10.0</option></select></label><label><span>Availability</span><select value={exploreTimeFilter} onChange={(event) => setExploreTimeFilter(event.target.value as ExploreTimeFilter)}><option value="all">Any time</option><option value="morning">Morning · 12 AM–9 AM</option><option value="day">Day · 9 AM–4 PM</option><option value="evening">Evening · 4 PM–9 PM</option><option value="night">Night · 9 PM–12 AM</option></select></label><label><span>Date</span><input type="date" value={exploreDateFilter} onChange={(event) => setExploreDateFilter(event.target.value)} /></label><button type="button" className="explore-filter-search-button" onClick={applyExploreFilters}>See results</button><button className="explore-filter-reset" type="button" onClick={() => { setExploreSearchDraft(""); setExploreSearch(""); setExploreSportFilter("all"); setExploreAreaFilter("all"); setExploreDateFilter(""); setExploreCmrFilter("all"); setExploreTimeFilter("all"); setExploreSportApplied("all"); setExploreAreaApplied("all"); setExploreDateApplied(""); setExploreCmrApplied("all"); setExploreTimeApplied("all"); }} disabled={!exploreSearch && exploreSportApplied === "all" && exploreAreaApplied === "all" && !exploreDateApplied && exploreCmrApplied === "all" && exploreTimeApplied === "all"}>Reset</button></div>}
        {gamesViewTab === "explore" && <div className="game-list">{exploreLoading ? <TennisBallLoader label="Finding nearby games" /> : filteredExploreGames.length ? filteredExploreGames.map((game) => <article className="game-row" key={game.id}><div className="game-date"><strong>{new Date(game.session_date).toLocaleDateString("en-IN", { weekday: "short" })}</strong><span>{new Date(game.session_date).getDate()}</span></div><div className="game-copy"><h2>{game.group_name}</h2><p>{sportLabel(game.sport)} · {game.session_date} · {game.start_time}–{game.end_time} · {game.area}</p><small className="waitlist-summary">{Math.round(game.score * 100)}% match · {game.open_slots} spot{game.open_slots === 1 ? "" : "s"} open</small></div><div className="game-row-actions"><button className="manage-group-button" onClick={() => void viewGroup(game.id)} disabled={loadingGroupId === game.id}>{loadingGroupId === game.id ? "Loading..." : "View group"}</button><button className="copy-link-button" onClick={() => void copyGroupSpaceLink(game)} aria-label={`Copy invite link for ${game.group_name}`} title="Copy invite link"><CopyIcon /><span>Copy invite link</span></button><button className="game-share-button" onClick={() => void shareGame(game)} aria-label={`Share ${game.group_name}`}>Share <span>↗</span></button>{game.organizer_id === user?.uid ? <span className="status-badge approved">Your group</span> : <button className="join-button" onClick={() => void joinSession(game.id, game.group_name, game.organizer_id)} disabled={joiningSessionId !== null || isSessionRequested(game.id)}>{joiningSessionId === game.id ? "Requesting..." : isSessionRequested(game.id) ? "Requested" : "Request to join"}<span>→</span></button>}</div></article>) : <div className="page-empty"><strong>No games match these filters.</strong><p>Try another area, sport, CMR, date, or availability.</p><button className="dark-button" onClick={() => { setExploreSportFilter("all"); setExploreAreaFilter("all"); setExploreDateFilter(""); setExploreCmrFilter("all"); setExploreTimeFilter("all"); setExploreSportApplied("all"); setExploreAreaApplied("all"); setExploreDateApplied(""); setExploreCmrApplied("all"); setExploreTimeApplied("all"); }}>Clear filters <span>→</span></button></div>}</div>}
        {!activityLoading && gamesViewTab === "upcoming" && <div className="game-list intuitive-game-list">{filteredUpcomingGames.length ? filteredUpcomingGames.map((game) => {
        ...
        }) : <div className="page-empty"><strong>{upcomingGames.length ? "No upcoming games match your search." : "No upcoming games yet."}</strong><p>{upcomingGames.length ? "Try another game name, sport, area, date, or time." : "Join a nearby game or create a game from Home."}</p>{upcomingGames.length ? <button className="dark-button" onClick={() => setUpcomingSearch("")}>Clear search <span>→</span></button> : <button className="dark-button" onClick={() => selectTab("home")}>Find a game <span>→</span></button>}</div>}</div>}
        {!activityLoading && gamesViewTab === "pending" && <div className="game-list intuitive-game-list">{filteredRequestedGames.length ? filteredRequestedGames.map(({ request, session }) => <article className={`game-row intuitive-game-card pending-game-card ${justRequestedSessionId === session.id ? "just-landed" : ""}`} key={request.id}>
        ...
        </article>) : <div className="page-empty"><strong>{requestedGames.length ? "No pending games match your search." : "No pending requests."}</strong><p>{requestedGames.length ? "Try another game name, sport, area, date, time, or status." : "Games you request will stay here until the organizer approves them."}</p>{requestedGames.length ? <button className="dark-button" onClick={() => setPendingSearch("")}>Clear search <span>→</span></button> : <button className="dark-button" onClick={() => selectTab("home")}>Find a game <span>→</span></button>}</div>}</div>}
        {!activityLoading && gamesViewTab === "awaiting_feedback" && <div className="completed-sections">
          {awaitingFeedbackGames.length > 0 && <section className="completed-action-section"><header><div><span className="kicker">ACTION NEEDED</span><h2>Finish your games</h2></div><strong>{awaitingFeedbackGames.length} remaining</strong></header><div className="game-list intuitive-game-list">{sortedAwaitingFeedbackGames.map((game) => <article className="game-row awaiting-feedback-row intuitive-game-card" key={game.id}><div className="game-date completed"><strong>{new Date(game.session_date).toLocaleDateString("en-IN", { weekday: "short" })}</strong><span>{new Date(game.session_date).getDate()}</span></div><div className="game-copy"><div className="game-card-title-line"><h2>{game.group_name}</h2><span className="needs-rating-label">Needs your rating</span></div><p>{sportLabel(game.sport)} · {game.session_date} · {game.area}</p><small className="waitlist-summary">Rate the players you met. Competitive CMR changes only after confirmed results.</small></div><div className="game-row-actions intuitive-card-actions completed-card-actions"><button className="manage-group-button primary-card-action" onClick={() => openPendingFeedback(game)}>Rate game <span>→</span></button><button className="game-share-button" onClick={() => { setPendingShareSessionId(game.id); void openGroupSpace(game); }}>Post to feed</button></div></article>)}</div></section>}
          {pastGames.length > 0 && <section className="completed-action-section completed-history-section"><div className="game-list intuitive-game-list completed-game-list">{sortedPastGames.map((pastGame) => <article className="game-row past-game-row intuitive-game-card" key={pastGame.session.id}><div className="game-date completed"><strong>{new Date(pastGame.session.session_date).toLocaleDateString("en-IN", { weekday: "short" })}</strong><span>{new Date(pastGame.session.session_date).getDate()}</span></div><div className="game-copy"><div className="game-card-title-line"><h2>{pastGame.session.group_name}</h2><span>Feedback saved</span></div><p>{sportLabel(pastGame.session.sport)} · {pastGame.session.session_date} · {pastGame.session.area}</p><small className="past-game-summary">{pastGame.rank ? `Ranked #${pastGame.rank} of ${pastGame.group_size}` : "Completed game"}{pastGame.score != null ? ` · ${pastGame.score.toFixed(1)} CMR` : ""}</small></div><div className="game-row-actions intuitive-card-actions completed-card-actions"><button className="manage-group-button primary-card-action" onClick={() => openPendingFeedback(pastGame.session)}>Rate game <span>→</span></button><button className="game-share-button" onClick={() => { setPendingShareSessionId(pastGame.session.id); void openGroupSpace(pastGame.session); }}>Post to feed</button></div></article>)}</div></section>}
      ...
      {howItWorksOpen && user && <section className="utility-page how-it-works-page" aria-labelledby="how-it-works-title">
        <div className="utility-page-header"><button className="utility-back-button" type="button" onClick={closeUtilityPage} aria-label="Go back">←</button><div><span className="kicker">THE COURTMATE LOOP</span><h1 id="how-it-works-title">How it works</h1><p>From “I want to play” to your 1–10 sport CMR.</p></div></div>
        <div className="how-it-works-intro"><span className="kicker">ONE SIMPLE LOOP</span><h2>Find. Play. Improve.</h2><p>Everything before and after a game stays together in CourtMate.</p></div>
        <div className="how-it-works-grid" aria-label="CourtMate game journey">
          <section><span>01</span><h2>Find or create</h2><p>Ask CourtMate or explore nearby games.</p></section>
          <section><span>02</span><h2>Join your group</h2><p>Confirm the lineup and plan in chat.</p></section>
          <section><span>03</span><h2>Play the game</h2><p>Keep every session in your history.</p></section>
          <section className="how-it-works-cmr"><span>04 · COURTMATE RATING</span><h2>Build your 1–10 CMR</h2><p>After a completed game, confirmed players rate one another from 1 to 10. CourtMate combines that feedback into a separate CMR for each sport.</p></section>
        </div>
        <div className="how-it-works-actions"><button className="dark-button" type="button" onClick={() => selectTab("home")}>Ask CourtMate <span>→</span></button><button className="outline-button" type="button" onClick={() => { setGamesViewTab("explore"); selectTab("games"); }}>Explore the map</button></div>
      </section>}

      {activeTab === "leaderboard" && user && profile && <section className="page-view circle-leaderboard-page" aria-label="Leaderboard">
        <div className="circle-leaderboard-tabs" role="tablist" aria-label="Leaderboard scope"><button type="button" className={circleLeaderboardScope === "circle" ? "active" : ""} onClick={() => setCircleLeaderboardScope("circle")} role="tab" aria-selected={circleLeaderboardScope === "circle"}>Core circle</button><button type="button" className={circleLeaderboardScope === "locality" ? "active" : ""} onClick={() => setCircleLeaderboardScope("locality")} role="tab" aria-selected={circleLeaderboardScope === "locality"}>Your locality</button><button type="button" className={circleLeaderboardScope === "bengaluru" ? "active" : ""} onClick={() => setCircleLeaderboardScope("bengaluru")} role="tab" aria-selected={circleLeaderboardScope === "bengaluru"}>Bengaluru</button></div>
        <div className="circle-leaderboard-metrics" role="tablist" aria-label="Leaderboard type"><button type="button" className={circleLeaderboardMetric === "sessions" ? "active" : ""} onClick={() => setCircleLeaderboardMetric("sessions")} role="tab" aria-selected={circleLeaderboardMetric === "sessions"}>Games played</button><button type="button" className={circleLeaderboardMetric === "cmr" ? "active" : ""} onClick={() => setCircleLeaderboardMetric("cmr")} role="tab" aria-selected={circleLeaderboardMetric === "cmr"}>CMR Ranking</button></div>
        <div className="circle-leaderboard-filters">{circleLeaderboardMetric === "sessions" ? <select value={circleLeaderboardGamesSport} onChange={(event) => setCircleLeaderboardGamesSport(event.target.value as Sport | "all")} aria-label="Games played sport"><option value="all">All sports</option>{sportOptions.map((sport) => <option value={sport.value} key={sport.value}>{sport.label}</option>)}</select> : <select value={circleLeaderboardSport} onChange={(event) => setCircleLeaderboardSport(event.target.value as Sport)} aria-label="CMR sport">{sportOptions.map((sport) => <option value={sport.value} key={sport.value}>{sport.label}</option>)}</select>}<select value={circleLeaderboardPeriod} onChange={(event) => setCircleLeaderboardPeriod(event.target.value as CircleLeaderboardPeriod)} aria-label="Leaderboard timeframe"><option value="week">This week</option><option value="30_days">Last 30 days</option><option value="3_months">Last 3 months</option><option value="year">Last year</option></select></div>
        <div className="circle-leaderboard-summary"><div><span className="kicker">{circleLeaderboardMetric === "sessions" ? circleLeaderboardGamesSport === "all" ? "ALL SPORTS" : sportLabel(circleLeaderboardGamesSport).toUpperCase() : sportLabel(circleLeaderboardSport).toUpperCase()}</span><h2>{circleLeaderboardScope === "circle" ? "Your people" : circleLeaderboardScope === "locality" ? profile.area || "Your locality" : "Across Bengaluru"}</h2></div><span>{circleLeaderboardEntries.length} ranked</span></div>
        {circleLeaderboardLoading ? <div className="connections-loader"><TennisBallLoader label="Building the table" detail={circleLeaderboardMetric === "sessions" ? `Counting ${circleLeaderboardGamesSport === "all" ? "all" : sportLabel(circleLeaderboardGamesSport)} game sessions...` : `Ranking ${sportLabel(circleLeaderboardSport)} CMR...`} /></div> : circleLeaderboardError ? <div className="utility-empty"><h2>Leaderboard unavailable</h2><p>{circleLeaderboardError}. Try again in a moment.</p><button className="dark-button" type="button" onClick={() => void loadCircleLeaderboard(circleLeaderboardScope, circleLeaderboardMetric, circleLeaderboardGamesSport, circleLeaderboardSport, circleLeaderboardPeriod)}>Try again <span>→</span></button></div> : circleLeaderboardEntries.length ? <div className="circle-leaderboard-list" role="tabpanel">{circleLeaderboardEntries.map((entry) => <button type="button" className={`circle-leaderboard-row rank-${entry.rank} ${entry.player.id === user.uid ? "current" : ""}`} onClick={() => void viewPlayerProfile(entry.player.id)} key={entry.player.id}><span className="circle-leaderboard-rank"><small>RANK</small>{entry.rank}</span><span className="circle-leaderboard-avatar">{entry.player.profile_image_url ? <img src={entry.player.profile_image_url} alt="" /> : initials(entry.player.display_name)}</span><span className="circle-leaderboard-player"><strong>{entry.player.display_name}{entry.player.id === user.uid && <em>You</em>}</strong><small>{entry.player.area || "Bengaluru"} · {entry.ratings_count} {circleLeaderboardMetric === "sessions" ? `game session${entry.ratings_count === 1 ? "" : "s"}` : `rated game${entry.ratings_count === 1 ? "" : "s"}`}</small></span><span className="circle-leaderboard-score"><b>{circleLeaderboardMetric === "sessions" ? entry.score.toFixed(0) : entry.score.toFixed(2)}</b><small>{circleLeaderboardMetric === "sessions" ? "GAMES" : "CMR"}</small></span></button>)}</div> : <div className="utility-empty circle-leaderboard-empty"><h2>{circleLeaderboardMetric === "sessions" ? `No completed ${circleLeaderboardGamesSport === "all" ? "games" : sportLabel(circleLeaderboardGamesSport) + " games"} here yet` : `No ${sportLabel(circleLeaderboardSport)} ratings here yet`}</h2><p>{circleLeaderboardMetric === "sessions" ? "Completed game sessions in this timeframe will appear on the leaderboard." : circleLeaderboardScope === "circle" ? "Connect with players and complete rated games to build this circle." : "Completed rated games will place players on this leaderboard."}</p></div>}
      </section>}
