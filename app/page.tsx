...
function PublicProfileView({ profile, followLoading, onBack, onFollow, onShare, onToast }: { profile: PublicPlayerProfile; followLoading: boolean; onBack: () => void; onFollow: () => void; onShare: () => void; onToast: (message: string) => void }) {
  const firstRatedSport = sportOptions
    .filter((sport) => (profile.cmr_game_counts?.[sport.value] ?? 0) > 0)
    .sort((left, right) => (profile.cmr_game_counts?.[right.value] ?? 0) - (profile.cmr_game_counts?.[left.value] ?? 0))[0]?.value ?? null;
  const [selectedSport, setSelectedSport] = useState<Sport | null>(firstRatedSport);
...
export default function Home() {
  ...
  function localPerformanceAnswer(requestQuery: string) {
    ...
    if (/\b(reliability|reliable|attendance|show up|show-up)\b/.test(normalized)) {
      ...
      return `Your CourtMate reliability is ${Math.round((profile?.reliability ?? 0) * 100)}%. You have ${showUps} confirmed show-up${showUps === 1 ? "" : "s"} and ${profile?.withdrawal_count ?? 0} withdrawal${profile?.withdrawal_count === 1 ? "" : "s"} recorded.`;
    }
