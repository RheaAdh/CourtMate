  ...
  return (
    <main className={`shell ${settingsOpen ? "settings-open" : ""}`}>
      ...
      {activeTab === "profile" && <section className="page-view profile-page">
        {user && profile && <section className="profile-photo-card"><div className="profile-photo-avatar">{profile.profile_image_url ? <img src={profile.profile_image_url} alt={`${profile.display_name} profile`} /> : profile.display_name[0].toUpperCase()}</div><div className="profile-photo-copy"><strong>{profile.display_name}</strong><label className="profile-photo-upload"><input type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadProfilePicture(file); event.currentTarget.value = ""; }} disabled={profilePictureUploading} />{profilePictureUploading ? "Uploading..." : profile.profile_image_url ? "Change photo" : "Add profile photo"}</label></div></section>}
