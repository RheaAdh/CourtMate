...
function communityKey(sport: Sport, area: string) { return `${sport}:${area.toLowerCase()}`; }
...
export function CommunityHub({ apiUrl, authorizedFetch, sessions, currentUserId, currentCmr, gamesLogged, initialLatitude, initialLongitude, initialArea = "Whitefield", loading, onJoinGame, onViewGroup }: CommunityHubProps) {
  ...
  useEffect(() => {
    let cancelled = false;
    void authorizedFetch(`${apiUrl}/v1/me/communities`).then(async (response) => {
      if (!response.ok) throw new Error();
      return response.json() as Promise<{ memberships: Array<{ sport: Sport; area: string }> }>;
    }).then((payload) => {
      if (!cancelled) setJoinedCircles(payload.memberships.map((membership) => communityKey(membership.sport, membership.area)));
    }).catch(() => {
      if (!cancelled) setJoinedCircles([]);
    });
    return () => { cancelled = true; };
  }, [apiUrl]);

  const joinCommunity = async (sport: Sport, area: string) => {
    const key = communityKey(sport, area);
    if (joinedCircles.includes(key) || joiningCircle === key) return;
    setJoiningCircle(key);
    try {
      const response = await authorizedFetch(`${apiUrl}/v1/me/communities/join`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ sport, area }),
      });
      if (!response.ok) throw new Error();
      setJoinedCircles((ids) => ids.includes(key) ? ids : [...ids, key]);
    } finally {
      setJoiningCircle((current) => current === key ? null : current);
    }
  };
