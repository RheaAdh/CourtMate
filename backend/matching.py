...
def search_sessions(sessions: list[Session], query: SearchIntent, players: list[Player] | None = None, player: Player | None = None) -> list[SessionRecommendation]:
    ...
    for session in sessions:
        ...
        if skill_fit < 1.0:
            continue
        if player and player.dupr_rating is not None and not session.skill_min <= player.dupr_rating <= session.skill_max:
            continue
        ...
        reasons = RecommendationReason(skill_fit=skill_fit, availability_fit=time_fit, area_fit=area_fit, style_fit=style_fit, reliability=member_reliability, familiarity=familiarity, explanation=f"{session.group_name} matches your requested area, time, and DUPR band with {session.open_slots} open slot(s).")
